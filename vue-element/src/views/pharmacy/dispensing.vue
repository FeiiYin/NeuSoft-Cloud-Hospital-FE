  ...
  methods: {
    ...
    invokeDispensing() {
      ...
      if (this.multipleSelectionChargeFormTable.length === 0) {
        // 这个也是异步执行的，需要注意
        this.$confirm('未选中任意药品, 是否将全部能发放的药品发放?', '提示', {
          ...
          for (i = 0; i < this.chargeFormTableList.length; ++i) {
            if (this.chargeFormTableList[i].payState === 1 && this.chargeFormTableList[i].notGivenNums > 0) {
              tempList.push({
                'prescriptionEntryId': this.chargeFormTableList[i].entryId,
                'executionNums': this.chargeFormTableList[i].notGivenNums
              })
            }
          }
          console.log(tempList)
          for (i = 0; i < tempList.length; ++i) {
            for (var j = 0; j < this.chargeFormTableList.length; ++j) {
              if (tempList[i].prescriptionEntryId === this.chargeFormTableList[j].entryId) {
                this.chargeFormTableList[j].notGivenNums = 0
                break
              }
            }
          }
        ...
        for (i = 0; i < this.multipleSelectionChargeFormTable.length; ++i) {
          if (this.multipleSelectionChargeFormTable[i].payState === 1 && this.multipleSelectionChargeFormTable[i].notGivenNums > 0) {
            tempList.push({
              'prescriptionEntryId': this.multipleSelectionChargeFormTable[i].entryId,
              'executionNums': this.multipleSelectionChargeFormTable[i].notGivenNums
            })
          }
        }
        if (tempList.length === 0) {
          this.$message.error('选中药品中均不能发放，错误！')
